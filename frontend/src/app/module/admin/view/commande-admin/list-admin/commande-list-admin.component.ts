...
export class CommandeListAdminComponent implements OnInit {
    ...
    constructor(private datePipe: DatePipe, private commandeService: CommandeService, private messageService: MessageService, private confirmationService: ConfirmationService, private roleService: RoleService, private router: Router, private authService: AuthService
    ...
    initDuplicateCommande(res: CommandeVo) {
        ...
        if (res.commandeItemsVo != null) {
            res.commandeItemsVo.forEach(d => {
                d.commandeVo = null;
                d.id = null;
            });
        }
        if (res.commandeTagsVo != null) {
            res.commandeTagsVo.forEach(d => {
                d.commandeVo = null;
                d.id = null;
            });
        }
